...
class MultiAICLData(object):
    ...
    def evaluate(self, predictions, groundtruths, is_classification):
        assert len(predictions)==len(self.multidata)
        accs = []
    ...
    def tensorize(self, _train_data, _test_data, options=None,
        ...
        for dp_idx, dp in enumerate(test_data):
            ...
            multidata.append({"indices": indices, "answer": answer, "options": dp["options"]})
...
#        all_tensorize_paths = [tensorize_path % i for i in range(1)]
