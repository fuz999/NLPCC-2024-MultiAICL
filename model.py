...
#        self.device1= torch.device("cuda:3")
 ...
 #   def to_device1(self):
  #      self.model.to(self.device1)
    ...
    def load(self, checkpoint=None, gpt2="gpt2-xl"): #gpt2-xl
        '''
    ...
    def save(self, step):
        if self.local_rank <= 0:
            ...
            with open(config_path, "w", encoding="utf-8") as f:
                json.dump(self.model.config.to_dict(), f, indent=4, ensure_ascii=False)

            self.logger.info("Saving model parameters at step=%d" % step)
            self.logger.info("Saving model configuration as config.json")
    ...
    def parallel(self):
        ...
        if self.local_rank != -1:
            self.model = torch.nn.parallel.DistributedDataParallelParallel(
    ...
    def do_predict(self, data, batch_size=1, losses=None, verbose=False):
        ...
        predictions = []
        for idx, dp in enumerate(data.multidata):
